...
- 已实现：`/health`、`/api/v1/cluster/status`、`/api/v1/nodes`、`/api/v1/nodes/:id (GET/DELETE)`、`/api/v1/services (GET/POST)`、`/api/v1/services/:name (GET)`。
- 已实现安全策略：leader 驱逐默认拒绝，需显式 `force=true`。
- 待实现：`/api/v1/services` 的更新冲突控制（CAS/resourceVersion）。
